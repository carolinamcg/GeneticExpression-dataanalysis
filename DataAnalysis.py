...
scores = cross_val_score(sgd_clf, train_in, train_out, cv=5)
...
scores = cross_val_score(neural_clf, train_in, train_out, cv=5)
print('Scores com cross validation com função scoring Pecc e com 5 folds:')
print(scores.mean()*100, '%')
...
scores = cross_val_score(model_svm, train_in, train_out, cv = 5)
print(scores)
...
# **Modelo de Classificação: Linear Regression**
...
lr = linear_model.LinearRegression() #LinearRegression fits a linear model with coefficients w = (w1, …, wp) 
